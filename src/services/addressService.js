...
import { ResourceNotFoundError, ValidationError } from '../../helpers/customError';

import userAddressMappingService from './userAddressMappingService';
import companyAddressMappingService from './companyAddressMappingService';
import billingAddressMappingService from './billingAddressMappingService';
...
addressService.findAllForSearch = (options, auth) => {
  const addressController = new AddressController(auth.customerId);
  return addressController.findAllForSearch(options);
};
...
addressService.create = async (values, auth) => {
  ...
  if (addressRetObj.currentApprovalId) {
    ...
    grootService.sendAddressEmailToApprover(addressRetObj.id, auth, userIds);
  }
...
addressService.findOneByIdForView = async (addressId, auth, options) => {
  const addressController = new AddressController(auth.customerId);
  const add = await addressController.findOneByIdForView(addressId, options);
  if (!add) {
    throw new ResourceNotFoundError('Address');
  }
  const address = add.toJSON();
...
addressService.sendLowStockNotifications = async (addressId, auth) => {
  grootService.sendLowStockNotifications(addressId, auth);
};
...
addressService.approve = async (addressId, auth) => {
  ...
  grootService.sendAddressApprovedEmailToUser(addressRetObj.id, auth);
  if (addressRetObj.currentApprovalId) {
    const userIds = addressRetObj.CurrentApproval.approvers.map(a => a.userId);
    await notificationService.bulkCreateForUsers({
      text: `${addressRetObj.keyword}: Address has been sent for approval by ${addressRetObj.User.name}`,
...
addressService.reject = async (values, addressId, auth) => {
  ...
  grootService.sendAddressRejectedEmailToUser(addressRetObj.id, auth);
  return { id: addressRetObj.id };
};
